# SIGMA v2.4 — Carta de Fundación del Ecosistema
...
Este documento distingue cuatro capas, sin mezclarlas:
...
3. **Marco interpretativo** — el estado real de interoperabilidad (dato
   verificado, punto 1) leído a través del Álgebra Axiométrica: no es
   un hecho nuevo ni una promesa de diseño, es una lente de lectura
   sobre datos ya confirmados, con desarrollo matemático propio fuera
   del alcance técnico de este documento. Sección "Interoperabilidad —
   más allá del binario implementado/no implementado".
4. **Prospectiva** — Hito 3: dirección de intención, no diseño cerrado.
...
| 02 | AGENTS_CREATOR.md | 1.3.0 | ✅ Operativo — §8/§9 actualizados (precisión Ag-DR + tabla de interoperabilidad de tres estados) |
| 03 | SIGMA_v2.3.md — Carta de Fundación del Ecosistema (este documento) | 2.3 | ✅ Operativo |
...
| ADR-016 | Orquestación Jerárquica (Director/Engineer/Auditor) | 1.3 | ✅ Aceptado — 4 condiciones de Rollout 1 verificadas en la práctica |
| ADR-017 | Sandboxing de Ejecución | 1.3 | ✅ Aceptado |
| ADR-018 | Memoria Operativa entre Corridas (Ag-DR) | 1.2 | ✅ Aceptado — §2.8 nueva (regla Juez↔HITL) |
| ADR-019 | Identidad de Agentes (CLOE, jerarquía unificada) | 1.6 | ✅ Aceptado |
...
| ADR-021 | Alineación Voluntaria con SR 26-02 (Gestión de Riesgo de Modelos) | 1.0 | ✅ Aceptado — documental, sin reclamo de cumplimiento |
| ADR-022 | RAG del Director sobre Documentación Interna (ChromaDB) | 1.0 | ✅ Aceptado — implementación prospectiva, Hito 3 |
| ADR-023 | Embeddings sobre Ag-DR para Detección de Patrones | 1.0 | ✅ Aceptado — implementación prospectiva, Hito 3 |
...
**Consolidado en la práctica; cierre formal pendiente del commit
agrupado** que reúne la migración de variantes, el fix de
`skill_runner.py`, ambos READMEs y los ADRs de esta sesión — sin ese
commit, Rollout 1 no está cerrado en los hechos, solo verificado. Las 4
condiciones de `ADR-016` Tab. 2 ya están confirmadas en la práctica, no
solo en diseño:
...
- **a** Aprobación firme de `ADR-016`, `ADR-017`, `ADR-018` (v1.2),
  `ADR-019` — los cuatro pasan de "Propuesto"/"Pre-aprobado" a
  "Aceptado". `ADR-018` v1.2 añade §2.8: regla de interacción entre el
  futuro Juez de Rollout 3 y el Ag-DR — puede enriquecer, nunca puede
  suprimir un disparador HITL determinista.
- **b** Tres ADRs nuevos: `ADR-021` (alineación voluntaria de
  principios con SR 26-02, explícitamente sin reclamo de cumplimiento
  — la propia guía excluye IA agéntica de su alcance), `ADR-022` (RAG
  del Director sobre documentación interna vía ChromaDB, implementa
  `Director.enter_research_mode()` de `ADR-019` §2.9), `ADR-023`
  (embeddings sobre Ag-DR para detección de patrones, agente-facing,
  extensión funcional de `ADR-018`). Los tres con implementación
  prospectiva (Hito 3), diseño aceptado ahora.
- **c** Cierre de Rollout 1 matizado: "consolidado en la práctica",
  cierre formal condicionado al commit agrupado pendiente — antes decía
  "Cerrado" sin esa condición.
- **d** `ESTRUCTURA_PROYECTO.md` programado explícitamente al cierre de
  Rollout 3; checkpoint de portafolio (LinkedIn + Kaggle) programado al
  cierre de Rollout 2 — ambos antes estaban sin fecha.

### Cambios en v2.3

- **a** Documento renombrado a "Carta de Fundación del Ecosistema" —
  se retira la comparación con "White Paper"; el documento mezcla voz
  institucional (capas 1-2) con voz de autor explícita (capas 3-4), lo
  cual no corresponde a un white paper convencional.
- **b** Nota de alcance ampliada de tres a cuatro capas — se añade
  "Marco interpretativo" como categoría propia para la tabla de
  Axiometría, distinguiéndola de dato verificado, diseño planeado, y
  prospectiva personal.
- **c** Resueltos, en paralelo con `AGENTS_CREATOR.md` v1.3.0, los dos
  primeros ítems de "Pendiente de decisión" de v2.0/v2.1/v2.2 — ver
  sección actualizada más abajo. Queda solo `ESTRUCTURA_PROYECTO.md`
  como pendiente real.
...
## Agenda de cierre — programado, no abierto sin fecha

1. **`ESTRUCTURA_PROYECTO.md`** — actualización del árbol de carpetas y
   el orden de operaciones (sigue mostrando `orchestrator.py` y
   `--variant Full/Dev`) programada explícitamente para el **cierre de
   Rollout 3**, no antes — es cuando la estructura real de carpetas
   (Engineer Modelos + Engineer Auditor + memoria RAG) deja de cambiar
   con cada Rollout.
2. **Checkpoint de portafolio** — sincronizar el artículo de LinkedIn y
   el notebook de Kaggle con el estado real del proyecto, programado al
   **cierre de Rollout 2** — es lo primero que un reclutador revisa
   antes que el repo completo.

*Resueltos en esta versión, en paralelo con `AGENTS_CREATOR.md` v1.3.0:*
- ~~`AGENTS_CREATOR.md` §8~~ — corregida: "nunca memoria mutable
  compartida en vivo" matizado como comportamiento por defecto *dentro*
  de una corrida; Ag-DR (`ADR-018`) documentado como excepción gated
  por aprobación humana (§2.5).
- ~~`AGENTS_CREATOR.md` §9~~ — tabla binaria reemplazada por versión de
  tres estados (esquema vs. transporte en MCP y A2A), con puente a este
  documento para el marco de cuatro valores.
