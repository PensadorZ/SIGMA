...
Corregida la mención de las 4 variantes de costo en el Contexto, que
usaba el esquema viejo ("SIGMA Dev, Lite, Full o Runtime" como si
fueran categorías planas) — ahora refleja los dos ejes reales: variante
de costo (SIGMA-FE/LE/ME/HE) y submodo (Dev/Runtime), independientes.
...
escrito una sola vez, se ejecute sin modificación en cualquier variante de costo (SIGMA-FE, LE, ME, HE)
y en cualquier submodo (Dev, Runtime) — sustituyendo en tiempo de ejecución cualquier valor que
...
- **a** Corregida la mención de variantes en el Contexto al esquema
  real (SIGMA-FE/LE/ME/HE + submodo Dev/Runtime, dos ejes
  independientes), reemplazando el esquema plano heredado.
