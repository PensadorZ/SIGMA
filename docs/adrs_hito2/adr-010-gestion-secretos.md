...
Corregida la referencia a `INSTALL.md` en el protocolo de rotación de
secretos — ese archivo todavía no existe (confirmado contra el
`README.md` ya actualizado en Hito 2), se señala dónde consultar el
orden real de reinicio mientras tanto (`ESTRUCTURA_PROYECTO.md`).
...
en `INSTALL.md`. **Nota real (Hito 2):** `INSTALL.md` todavía no existe
como archivo — el `README.md` ya lo documenta como limitación conocida.
Mientras no exista, el orden de reinicio vive en
`ESTRUCTURA_PROYECTO.md`, que sí es real y está actualizado; este ADR
no crea `INSTALL.md` por su cuenta, solo señala dónde consultar el
orden real hasta que se escriba.
...
- **a** Corregida la referencia a `INSTALL.md` (no existe todavía) en
  el protocolo de rotación de secretos — señalado `ESTRUCTURA_PROYECTO.md`
  como fuente real mientras tanto, sin crear el archivo por cuenta propia.
