...
supersede: ADR-013 v1.4
referencias-minimas: ADR-003, ADR-005, ADR-008, ADR-011, ADR-016, ADR-018
...
Corrección real: el cuerpo del documento seguía citando la referencia
rota "ADR-00" pese a que el propio changelog de v1.4 afirmaba haberla
corregido a ADR-016 — se aplica ahora de verdad, junto con un texto
incompleto ("tales como...") que quedó sin terminar en una edición
anterior. Se añade vínculo con ADR-018: el Ag-DR usa exactamente los
mismos campos que este ADR ya definía (`run_id`, `skill_id`, `verdict`)
— no son mecanismos independientes.
...
El agente Auditor (definido formalmente en ADR-016 como el tercer
orquestador del patrón Director/Engineer/Auditor) verifica la
trayectoria real de cada ejecución contra la trayectoria esperada
declarada en el `SKILL.md` de ADR-009, usando las trazas de Langfuse V2
de ADR-011 como fuente de verdad.
...
- **a** Aplicada de verdad la corrección de "ADR-00" → "ADR-016" que
  el changelog de v1.4 ya afirmaba haber hecho, sin que el cuerpo del
  documento reflejara el cambio.
- **b** Corregido texto incompleto ("tales como...") sobre el origen
  de las trazas — queda fijo en Langfuse V2 (ADR-011), sin alternativas
  sin especificar.
- **c** Añadido vínculo con ADR-018 (Ag-DR) — comparten campos
  (`run_id`, `skill_id`, `verdict`), no son mecanismos redundantes.
