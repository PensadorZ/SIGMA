...
supersede: ADR-009 v1.7
referencias-minimas: ADR-002, ADR-006, ADR-011, ADR-014, ADR-016, ADR-017
aprobado-por: Prof. Marx A. García Delgado
---

# ADR-009: Especificación de Skills con Gherkin, LTL y Estructura Granular

## Resumen ejecutivo de cambios v1.8

Dos correcciones reales, verificadas contra el estado actual del
proyecto (Hito 2, cierre de Rollout 1): (1) Tab. 5 todavía decía
"Orquestador en modo Arquitecto" como autor de skills generados
dinámicamente — terminología previa a la jerarquía Director/Engineer/
Auditor, ya reconciliada en `ADR-014` v1.2 pero nunca propagada aquí;
(2) Tab. 2b agrupaba incorrectamente `0004` dentro del cierre original
de Hito 1 (55 tests), cuando en realidad se construyó durante Hito 2
Rollout 1 — el conteo de 65/65 solo existe después de esa adición. Se
añaden `ADR-014`, `ADR-016` y `ADR-017` a las referencias mínimas,
citados directamente en el cuerpo pero ausentes de la lista.
...
**Corregido:** la versión anterior agrupaba `0004` dentro de "Hito 1,
65/65 tests" — pero `0004-statistical-validator` se construyó durante
Hito 2 Rollout 1, no en el cierre original de Hito 1 (que cerró con 6
skills y 55 tests). El conteo de 65/65 solo existe *después* de que
Rollout 1 de Hito 2 añadió `0004`. Se separa correctamente abajo, y se
desglosan los Rollouts de Hito 2 según `ADR-016` en vez de un bloque
plano "Especificación".

| Rango | Skills | Hito / Rollout | Estado |
|---|---|---|---|
| 0000–0003, 0008, 0011 | Pipeline batch core + sentiment + viz | Hito 1 (cierre original) | ✅ Implementados (55/55 tests en el cierre) |
| 0004 | Validación estadística (Engineer Datos) | Hito 2, Rollout 1 | ✅ Implementado — suite conjunta 65/65 (con los 6 de Hito 1) |
| 0005–0007, 0009–0010 | ML/DL trainers, clustering, engagement (Engineer Modelos) | Hito 2, Rollout 2 | Especificación — sin código todavía |
| 0012–0015 | HITL, explainability, code-reviewer, skill-discovery (Engineer Auditor) | Hito 2, Rollout 3 | Especificación — gateado por `ADR-017` (sandboxing) |
...
| Generado por IA (Director autoriza, Engineer de dominio genera) | `gia_` + SemVer | `gia_0.1.0` |

El prefijo `gia_` significa **Generado por Inteligencia Artificial**. Los
skills con esta marca han sido producidos por el Engineer del dominio
correspondiente (Datos/Modelos/Auditor), bajo autorización del Director,
siguiendo el flujo de generación dinámica definido en `ADR-014` v1.2 —
**corregido**: la versión anterior de este documento decía "Orquestador
en modo Arquitecto", terminología previa a la jerarquía real
Director/Engineer/Auditor de `ADR-016`, ya reconciliada en `ADR-014`
pero que aquí había quedado sin actualizar. La marca es permanente
durante la vida del skill generado. Si un skill generado por IA es
rediseñado por completo por un humano, se considera un skill nuevo con
numeración y versión independientes.
...
**Cambios en v1.8 (Hito 2, cierre de Rollout 1):**
- **a** Tab. 5 corregida: "Orquestador en modo Arquitecto" → Director
  autoriza, Engineer del dominio correspondiente genera — consistente
  con la reconciliación ya aplicada en ADR-014 v1.2.
- **b** Tab. 2b corregida: `0004` separado del cierre original de
  Hito 1 (55 tests) — se construyó en Hito 2 Rollout 1. Rollouts 2 y 3
  de Hito 2 desglosados según ADR-016 en vez de un bloque plano.
- **c** Añadidas ADR-014, ADR-016, ADR-017 a referencias mínimas.
