...
supersede: ADR-012 v1.4
referencias-minimas: ADR-009, ADR-010, ADR-011, ADR-014
...
Añadida `ADR-014` a las referencias mínimas — su Paso 7 ("Versionado y
promoción") depende explícitamente del ciclo Dev → Staging → Production
que este documento define, sin que la relación estuviera declarada
formalmente aquí. Sin otros cambios: el resto del documento ya estaba
libre de esquema de variantes desactualizado y de terminología de
worker, verificado contra el estado real del proyecto en esta sesión.
...
- **a** Añadida ADR-014 a referencias mínimas — su ciclo de promoción
  de skills generados dinámicamente depende directamente de este ADR.
