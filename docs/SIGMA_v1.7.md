...
| 03 | SIGMA_v1.7.md (este documento) | 1.7 | ✅ Operativo |
...
| 08 | TROUBLESHOOTING.md | 1.1 | ✅ Operativo |
...
| ADR-016 | Orquestación Jerárquica de Tres Orquestadores (Director/Engineer) | 1.0 | 🔄 Propuesto |
...
| 0005 | `framework-selector` | Selecciona framework ML/DL por energía hamiltoniana | ⬜ Pendiente | Hito 2 |
...
> **Nota sobre 0016–0019 (reservados):** el ADR-015 reserva estos números para
> el Hito 3 de streaming — `hamilton-selector` (0016, priorización de mensajes
> en tiempo real) y tres consumidores Faust (0017–0019). El catálogo completo
> del ecosistema alcanzará 20 skills cuando se implementen.
...
Eje de clasificación: **costo operativo**, en cuatro escalones. Cada
variante puede además operar en dos submodos transversales, independientes
del costo: **Dev** (depuración, restricciones relajadas) y **Runtime**
(instancia desplegada con datos reales, MFA obligatorio).

```bash
SIGMA_VARIANT=FE       # SIGMA Full Engineer — costo $0
SIGMA_MODE=Dev         # submodo transversal opcional: Dev | Runtime | (vacío = estándar)
```

### Tabla comparativa por costo

| Criterio | SIGMA-FE | SIGMA-LE | SIGMA-ME | SIGMA-HE |
|---|---|---|---|---|
| Nombre completo | Full Engineer | Low-Cost Engineer | Medium-Cost Engineer | High-Cost Engineer |
| Costo operativo | $0 | Bajo | ~50% servicios pagos | Alto |
| Esfuerzo de ingeniería requerido | Muy alto | Alto | Medio | Bajo |
| Perfil recomendado | Ingeniero senior, dispuesto a documentarse en paralelo | Ingeniero con experiencia media | Equipo con presupuesto moderado | Empresa con necesidad de alto desempeño |
| Servicios premontados de pago | Ninguno | Solo esenciales | ~50% del stack | Mayoría del stack |
| Langfuse | Autoalojado | Autoalojado o Cloud básico | LangSmith / Cloud | LangSmith Enterprise |
| Almacenamiento | MinIO local | MinIO local | GCS/S3 gestionado | GCS/S3 con SLA |
| Orquestador LLM | Gemini free tier / Ollama | Gemini free tier | Gemini Pro | Vertex AI o equivalente enterprise |

### Descripción narrativa

**SIGMA-FE (Full Engineer)** — Costo $0. Requiere destreza real de
ingeniería: no se recomienda a aprendices o neófitos, salvo que estén
dispuestos a documentarse en paralelo mientras instalan el primer Hito.
Variante de referencia — todos los ADR activos, stack 100% autoalojado.

**SIGMA-LE (Low-Cost Engineer)** — Paga servicios premontados, pero solo
los esenciales, a bajo costo. Reduce la fricción de instalación sin
comprometer significativamente el presupuesto.

**SIGMA-ME (Medium-Cost Engineer)** — Utiliza aproximadamente el 50% de
sus servicios en modalidad de pago. Punto intermedio entre autogestión
completa y tercerización total.

**SIGMA-HE (High-Cost Engineer)** — Costos altos, reservada para empresas
que necesitan alto desempeño y soporte garantizado.

### Submodos transversales (aplican a cualquiera de las cuatro variantes)

**Dev** — Restricciones relajadas para no interrumpir la depuración:
workers forzados a 1 (ADR-002), SMOTE reemplazado por class_weight,
...
**Runtime** — La variante correspondiente desplegada con datos reales.
Requiere MFA con TOTP en el Approval Endpoint. Cualquier generación
dinámica de skills (ADR-014) requiere aprobación del operador
independientemente del nivel de impacto.

Ejemplo de combinación: `SIGMA-FE` en modo `Dev` es el entorno típico de
desarrollo local sin coste; `SIGMA-HE` en modo `Runtime` es un despliegue
empresarial de alto desempeño con datos de producción reales.
...
3. PRE-VUELO DE SEGURIDAD [Solo Runtime]
...
Las cuatro variantes de costo (SIGMA-FE/LE/ME/HE) tienen ambas capas activas
por defecto. El submodo **Dev** (transversal a cualquiera de las cuatro)
mantiene la capa estructural siempre activa, pero la capa semántica pasa a
opcional — para no interrumpir ciclos rápidos de depuración.
...
| Approval Endpoint (puerto 8765) | HITL para acciones críticas | ✅ Sí | Runtime |
...
|---|---|---|---|
| Orquestador (LangGraph) | Construye y ejecuta el DAG | ADR-002, ADR-009, ADR-016 | Todas |
| Agente Auditor | Verifica trayectoria, K⊆X y calidad | ADR-013 | Todas |
| Red Team | Ataque simulado en pre-vuelo | ADR-003 | Runtime |
| Blue Team | Monitorización de modelos (AgBOM) | ADR-003 | Todas excepto Dev |
...
| Approval Endpoint | HITL local con Vibe Diff persistente | ADR-004 | Runtime |
...
> Dev y Runtime son submodos transversales — no alteran esta tabla por sí
> mismos, salvo la reducción de RAM recomendada en Dev.

| Componente | SIGMA-FE | SIGMA-LE | SIGMA-ME | SIGMA-HE | Versión mínima |
|---|:---:|:---:|:---:|:---:|---|
| Python | ✅ | ✅ | ✅ | ✅ | 3.12.4 |
...
| RAM mínima | 16 GB | 12 GB | 8 GB | — (gestionado) | — |
| RAM recomendada | 32 GB | 24 GB | 16 GB | — (gestionado) | — |
...
├── SIGMA_v1.7.md                # Este documento — Plan Maestro
├── TROUBLESHOOTING.md          # Incidentes reales y su solución
...
cp .env.example .env
# Variables obligatorias: GEMINI_API_KEY, SIGMA_VARIANT=FE
...
|---|---|---|
| Google Cloud Trace | Langfuse V2 (autoalojado) | Exportable a Cloud Trace en variantes de mayor costo |
| Google Cloud Logging | Langfuse + logs locales | Fallback a Redis y archivos (ADR-011) |
| BigQuery | PostgreSQL + DuckDB | Reemplazable por BigQuery en SIGMA-ME/HE |
...
| LangGraph | LangGraph (orquestador de grafos) | Motor real del DAG de skills; circuit breaker con fallo rápido; formalizado en ADR-016 |
