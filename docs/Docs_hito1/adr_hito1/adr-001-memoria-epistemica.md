...
Modo lectura: workers acceden directamente al grafo (sin cola)
```

### Tab. 1 — Implementación del Feature Store por variante

| Variante | Backend | Coste adicional | Requisito de configuración |
|---|---|---|---|
| **SIGMA Full** | PostgreSQL con particionado temporal | Sin coste de licencia | Índice temporal manual |
| **SIGMA Lite** | Bigtable o DynamoDB (adaptador `FeatureStoreClient`) | Coste por operación | Credenciales cloud |
| **SIGMA Dev** | PostgreSQL con datos sintéticos | Sin coste de licencia | Idéntico a Full |
| **SIGMA Runtime** | PostgreSQL o Bigtable según entorno desplegado | Variable | Según elección |

### Componente 1 — Feature Store temporal

Almacena hechos verificados con versionado histórico. En SIGMA Full usa
...
el backend, lo que permite migrar de SIGMA Full a SIGMA Lite sin modificar
los skills.
...
**En SIGMA Full:** JSON versionado por entidad más NetworkX en memoria.
**En SIGMA Lite:** Neo4j AuraDB mediante adaptador `GraphClient`.

### Escritura serializada durante pipelines paralelos

Durante una ejecución activa con workers MapReduce, los workers encolan
...
- Dos sistemas de almacenamiento aumentan la complejidad operacional en
  SIGMA Full.
