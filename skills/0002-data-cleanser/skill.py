...
def run(state: PipelineState) -> SkillResult:
    with timer() as t:
        ...
        try:
            ...
            high_dup_threshold = float(
                cfg.get("deduplication", {}).get("high_duplicate_rate_threshold", 0.20)
            )
        ...
        warnings = []
        if num_exact_duplicates / max(num_input, 1) > high_dup_threshold:
