...
def _emit_langfuse(
    ...
    NOTA (corrección real, encontrada en pruebas de esta sesión): el SDK
    v4 instalado (4.13.0) cambió la firma de create_event(). Ya no acepta
    trace_id directo — exige trace_context={'trace_id': ...}, y ese
    trace_id debe ser un string hexadecimal de 32 caracteres (formato
    OTel), no el 'sigma-{fecha}-{hex}' que usa SIGMA internamente.

    Se deriva un trace_id de Langfuse válido de forma DETERMINÍSTICA a
    partir del trace_id real de SIGMA vía create_trace_id(seed=...) —
    mismo trace_id de SIGMA siempre produce el mismo trace_id de
    Langfuse, así que todos los eventos de una corrida quedan
    correlacionados en la misma traza visible en el dashboard.
    """
    try:
