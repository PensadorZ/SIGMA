...
def _run_skill(
    ...
    while True:
        ...
        if result["status"] in ("success", "success_with_warnings"):
            ...
            # Alerta HITL si pct_unclear > 30% en 0008
            if (
                skill_id == "0008"
                and result["output"].get("pct_unclear", 0) > 30.0
            ):
                log.warning("[hitl] pct_unclear=%.1f%% > 30%% → notificando Zulip",
                            result["output"]["pct_unclear"])
                notify_hitl(
                    message=(
                        f"⚠️ SIGMA Hito 1 — skill 0008: "
                        f"pct_unclear={result['output']['pct_unclear']:.1f}% "
                        f"supera el umbral del 30%. "
                        f"Revisa la calidad del corpus. "
...
def edge_after_0008(state: PipelineState) -> str:
    return _edge_after_skill(state, "node_0011")
...
def build_graph() -> StateGraph:
    ...
    DAG principal (acíclico):
      START → 0000 → 0001 → 0002 → 0003 → 0008 → 0011 → END
    ...
    ADR-011: LangGraph como motor de orquestación.
...
def main() -> None:
    ...
    final_state: PipelineState = compiled_graph.invoke(state)
