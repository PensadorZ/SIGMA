...
La clasificación crítico/opcional documentada en `defaults.yaml` es
referencial — la lógica real todavía vive hardcodeada en `skill.py`
(`_build_output`). Externalizarla por completo a configuración (para que
cambiar qué es crítico no requiera tocar código) queda como mejora futura,
no bloqueante para el Hito 1.
