...
def run(state: PipelineState) -> SkillResult:
    with timer() as t:
        ...
        if _db_url:
            ...
            log.info("[diag] DATABASE_URL vista por el proceso: ...@%s", _masked)
        else:
            log.info("[diag] DATABASE_URL: AUSENTE en os.environ de este proceso")
        log.info("[diag] REDIS_URL vista por el proceso: %s", _redis_url or "AUSENTE en os.environ de este proceso")

        import time as _time_diag

        _t0 = _time_diag.monotonic()
        pg_result = check_postgres(timeout_seconds=timeout)
        log.info("[diag] check_postgres tardó %.2fs (timeout configurado=%.1fs)",
                 _time_diag.monotonic() - _t0, timeout)

        _t0 = _time_diag.monotonic()
        redis_result = check_redis(timeout_seconds=timeout)
        log.info("[diag] check_redis tardó %.2fs", _time_diag.monotonic() - _t0)

        _t0 = _time_diag.monotonic()
        minio_result = _check_minio(timeout_seconds=timeout)
        log.info("[diag] _check_minio tardó %.2fs", _time_diag.monotonic() - _t0)

        _t0 = _time_diag.monotonic()
        langfuse_result = _check_langfuse(timeout_seconds=timeout)
        log.info("[diag] _check_langfuse tardó %.2fs", _time_diag.monotonic() - _t0)

        _t0 = _time_diag.monotonic()
        ollama_result = _check_ollama(timeout_seconds=timeout)
        log.info("[diag] _check_ollama tardó %.2fs", _time_diag.monotonic() - _t0)
