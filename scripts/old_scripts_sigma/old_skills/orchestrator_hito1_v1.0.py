...
logging.basicConfig(
    # SIGMA_LOG_LEVEL=DEBUG activa las líneas [diag] (tiempos por servicio,
    # valores reales de variables de entorno vistas por el proceso) sin
    # tener que volver a subirlas a INFO permanente. Default: INFO.
...
def _emit_langfuse(
    ...
    NOTA v1.1.2 — CORRECCIÓN DE FONDO (no solo de firma):
    Se descubrió que el 404 en `Failed to export span batch` no era un
    problema de puerto ni de firma — era una incompatibilidad real de
    versiones. El SDK v4 (4.13.0) envía trazas vía OTLP a la ruta
    `/api/public/otel`, que SOLO existe en servidores Langfuse ≥3.22.0.
    El servidor autoalojado de SIGMA está fijado deliberadamente a
    `ghcr.io/langfuse/langfuse:2` (v2) para evitar la dependencia de
    ClickHouse que exige la v3 — decisión ya tomada y que no se revierte
    aquí. Verificado con curl directo: la ruta OTLP devuelve 404 desde
    el propio manejador de "página no encontrada" de Next.js, es decir,
    ni siquiera existe como ruta de API en esta versión del servidor.

    Resolución: downgrade del SDK Python a `langfuse<3` (verificado
    contra 2.60.10), que usa la API legacy basada en REST
    (`.event()`, `.trace()`) sobre `/api/public/ingestion` — la ruta que
    SÍ existe en la v2 del servidor. Esta API además NO exige que
    trace_id sea hexadecimal de 32 caracteres — acepta cualquier string,
    así que el trace_id real de SIGMA se pasa tal cual, sin conversión.
    """
    try:
        langfuse.event(trace_id=trace_id, name=event_name, metadata=fields)
...
def _run_skill(
    ...
    log.debug("[diag] _emit_langfuse('%s.node_start') tardó %.2fs", skill_id, time.monotonic() - _t_diag)
