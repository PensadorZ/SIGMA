...
from sigma.skills._common import is_dev_mode, load_defaults, make_error, make_success, timer
    ...
    with timer() as t:
        try:
            if "df" not in state or state["df"] is None:
                raise InputSchemaError("Falta 'df' validado por 0002-data-cleanser")

            if mode == "drift":
                outcome = _run_drift_check(state["df"], state["baseline_df"], policies)
            elif mode == "leakage":
                outcome = _run_leakage_check(state["df"], state["leakage_target_col"], policies)
            else:
                ...
                if branch == "bayes_factor":
                    outcome = _run_bayes_factor(state["df"], state["hypothesis"], config, policies)
                elif branch == "adf_granger":
                    outcome = _run_adf_granger(state["df"], state["datetime_index"], config)
                elif branch == "bayesian_ab":
                    outcome = _run_bayesian_ab(state, config, policies)
                elif branch == "permutation_bootstrap":
                    outcome = _run_permutation_bootstrap(state["df"], config, policies)
                else:
                    outcome = _run_descriptive_fallback(state["df"], config)
