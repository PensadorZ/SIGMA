...
def _build_output(
    ...
    critical_checks = {"postgres": pg_result, "minio": minio_result}
    optional_checks = {"redis": redis_result, "langfuse": langfuse_result, "ollama": ollama_result}
    ...
    for name, result in optional_checks.items():
        services.append(ServiceStatus(
...
def run(state: PipelineState) -> SkillResult:
    with timer() as t:
        ...
        pg_result = check_postgres(timeout_seconds=DEFAULT_SERVICE_TIMEOUT_SECONDS)
        redis_result = check_redis(timeout_seconds=DEFAULT_SERVICE_TIMEOUT_SECONDS)
        minio_result = _check_minio(timeout_seconds=DEFAULT_SERVICE_TIMEOUT_SECONDS)
        langfuse_result = _check_langfuse(timeout_seconds=DEFAULT_SERVICE_TIMEOUT_SECONDS)
        ollama_result = _check_ollama(timeout_seconds=DEFAULT_SERVICE_TIMEOUT_SECONDS)
