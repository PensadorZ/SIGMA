...
def _load_model(model_path: str, dev: bool):
    ...
    # NOTA (bug real encontrado, 5 de julio de 2026): Path("").exists()
    # devuelve True en Windows -- interpreta la cadena vacía como el
    # directorio actual, que siempre existe. Esta verificación nunca se
    # disparaba cuando ROBERTA_MODEL_PATH estaba vacía/ausente, y el
    # código caía directo a AutoTokenizer.from_pretrained("") con un
    # error de HuggingFace confuso en vez de este mensaje claro.
    if not model_path or not Path(model_path).exists():
        raise ModelNotFoundError(
            f"Modelo RoBERTa no encontrado. ROBERTA_MODEL_PATH está "
            f"vacía o no configurada en tu .env (valor recibido: "
            f"{model_path!r}). Ejecuta scripts/download_model.py y "
            f"configura ROBERTA_MODEL_PATH con la ruta resultante."
        )
