...
# =============================================================================
# NOTA v1.0.2: Corrección de calidad visual en _build_dashboard_plotly():
#   1. Colores de sentimiento fijos por categoría (antes: Plotly reasignaba
#      colores según orden de aparición, inconsistente entre corridas).
#   2. Idiomas minoritarios agrupados en "otros" (bug real observado:
#      langdetect clasifica tweets cortos como docenas de idiomas exóticos
#      por error estadístico, generando un pie chart con 32+ porciones
#      diminutas cuyas etiquetas se apilaban ilegibles).
#   3. Eje X del gráfico de engagement usa row_id real en vez de índice
#      genérico 0..9 sin significado de negocio.
# No se modificó ninguna otra función de este archivo.
#
...
# Paleta fija de sentimiento — evita que Plotly reasigne colores según
# el orden de aparición de las categorías, que cambiaba entre corridas
# y rompía la consistencia semántica (verde=positivo debe significar lo
# mismo siempre, no depender de qué categoría aparece primero).
SENTIMENT_COLORS = {
    "POSITIVE": "#2ECC71",
    "NEGATIVE": "#E74C3C",
    "NEUTRAL": "#95A5A6",
...
_DEFAULT_SENTIMENT_COLOR = "#7F8C8D"

# Cuántos idiomas mostrar individualmente antes de agrupar el resto en "otros".
TOP_N_LANGS = 5
...
def _build_dashboard_plotly(df: pd.DataFrame, pre_aggregated: bool) -> tuple[str, int]:
    ...
    # ── Sentimiento con paleta fija (Fix #1) ────────────────────────────
    sentiment_counts = df["sentiment"].value_counts().reset_index()
    sentiment_counts.columns = ["sentiment", "count"]
    sentiment_colors = [
        SENTIMENT_COLORS.get(s, _DEFAULT_SENTIMENT_COLOR)
        for s in sentiment_counts["sentiment"]
    ]

    # ── Idiomas agrupados: top N + "otros" (Fix #2) ─────────────────────
    # Antes: mostrar las 32+ categorías que langdetect produce en tweets
    # cortos generaba un pie chart con etiquetas apiladas e ilegibles.
    lang_counts_full = df["lang"].value_counts()
    if len(lang_counts_full) > TOP_N_LANGS:
        top_langs = lang_counts_full.nlargest(TOP_N_LANGS)
        others_total = int(lang_counts_full.iloc[TOP_N_LANGS:].sum())
        lang_counts = top_langs.reset_index()
        lang_counts.columns = ["lang", "count"]
        lang_counts = pd.concat(
            [lang_counts, pd.DataFrame([{"lang": "otros", "count": others_total}])],
            ignore_index=True,
        )
    else:
        lang_counts = lang_counts_full.reset_index()
        lang_counts.columns = ["lang", "count"]

    # ── Top engagement con etiquetas reales (Fix #3) ────────────────────
    top_engagement = df.nlargest(min(10, len(df)), "engagement_score")
    if "row_id" in top_engagement.columns:
        engagement_labels = [str(rid)[:8] for rid in top_engagement["row_id"]]
    else:
        engagement_labels = [str(i) for i in range(len(top_engagement))]
    ...
    fig.add_trace(
        go.Bar(
            x=engagement_labels,
            y=top_engagement["engagement_score"],
            marker=dict(color="#3498DB"),
        ),
        row=1, col=2,
    )
    fig.add_trace(
        go.Pie(labels=lang_counts["lang"], values=lang_counts["count"]),
        row=1, col=3,
    )
