...
# Determinismo obligatorio (K ⊆ X) -- sin esto, langdetect puede variar
# su resultado para el mismo texto entre corridas distintas.
...
def _detect_lang(text: str) -> str:
    """
    Detección de idioma real vía langdetect (clasificador Naive Bayes,
    más de 55 idiomas soportados vía códigos ISO 639-1) -- reemplaza la
    heurística de palabras clave anterior (antes _detect_lang_simple),
    que solo distinguía es/en/und por conteo de palabras comunes, sin
    robustez real ante otros idiomas ni ante texto ambiguo.

    DetectorFactory.seed se fija a nivel de módulo para determinismo --
    sin eso, langdetect puede devolver resultados DISTINTOS para el
    MISMO texto en corridas distintas, lo cual violaría K ⊆ X (misma
    entrada observada debe producir siempre la misma salida).
    """
    cleaned = text.strip()
    if not cleaned:
        return "und"
...
def run(state: PipelineState) -> SkillResult:
    with timer() as t:
        ...
        df["lang"] = df["clean_text"].apply(_detect_lang)
